...
class BevShapeHead(nn.Module):
    ...
    def sigmoid(self, x):
        # y = torch.clamp(self.softmax(x), min=1e-4, max=1 - 1e-4)
    ...
    def get_loss_bev_shape(self):
        ...
        target_dicts = self.forward_ret_dict['target_dicts_BSH']
    ...
    def forward(self, data_dict):
        ...
        #     )
        #     self.forward_ret_dict['target_dicts_BSH'] = target_dict

        # self.forward_ret_dict['pred_dicts'] = pred_dicts
        # 这里应该是softmax
        data_dict['bev_hm'] = self.sigmoid(pred_dicts[0]['hm'])  #  # B，class,H,W torch.Size([2, 3, 160, 160])
        ...
        for i in range(data_dict['batch_size']):
            hm_prob_tensor = hm_prob[i, :, :, :]  #这个地方感觉是需要改一下的
        ...
        # hm_prob_fuse = []
        # for i in range(data_dict['batch_size']):
        #     hm_prob_tensor = hm_prob[i, 0, :, :]  #这个地方感觉是需要改一下的
        #     hm_prob_fuse.append(hm_prob_tensor.unsqueeze(0))

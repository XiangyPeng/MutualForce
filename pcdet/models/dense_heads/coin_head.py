...
class CoInHead(nn.Module):
    ...
    def get_loss(self):
        ...
        if type(LP_loss)==tuple:
            loss = 0.7 * LP_loss[0]+ loss
            tb_dict['LP_loss_head'] =  LP_loss[0].item()
        else:

            loss += 0.7 * LP_loss
            tb_dict['LP_loss_head'] = LP_loss.item()
        
        MCloss_temp = self.MCloss(feat, orignal_heatmap)
        if  MCloss_temp is not None:
            x_contrast_loss = 0.5 * MCloss_temp
    ...
    def non_none_reduce(self,func, seq):
        return reduce(lambda x, y: x + [y] if y is not None else x, seq, [])

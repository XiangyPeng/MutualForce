            ...
            if has_label:
                ...
                if count_inside_pts:
                    if self.pc_type == 'lidar' or self.pc_type == 'radar':
                        ...
                        calib = self.get_calib(sample_idx)
                        points = calib.lidar_to_rect(points[:, 0:3])
                    else:
                        # if sample_idx=="00544":
                        #     print("sdsd")
                        lidar_points, radar_points = self.get_pointcloud(sample_idx, self.pc_type)
                        # 这个是单独的呢还是所有呢？
                        points = self.get_lidar(sample_idx) # 这个应该是有问题的吧，应该是lidar和radar里面总共的点数目
                        calib = self.get_calib(sample_idx)
                        lidar_points = calib.lidar_to_rect(lidar_points[:, 0:3])
                        radar_points = calib.lidar_to_rect( radar_points[:, 0:3])
                    if self.pc_type == 'lidar' or self.pc_type == 'radar':
    ...
    def __getitem__(self, index):
        ...
        if "points" in get_item_list:
            if self.pc_type == 'lidar' or self.pc_type == 'radar':
                ...
                if self.dataset_cfg.FOV_POINTS_ONLY:
                    pts_rect = calib.lidar_to_rect(radar_points[:, 0:3])
                    fov_flag = self.get_fov_flag(pts_rect, img_shape, calib)

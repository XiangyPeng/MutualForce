...
class KittiDataset(DatasetTemplate):
    ...
    def get_sensor_transforms(self,  idx,sensor: str):  # -> Optional[(np.ndarray, np.ndarray)]:
        """
        ...
        if sensor == 'radar':
            calibration_file =  self.root_split_path / 'radar_calib' / ('%s.txt' % idx)


        elif sensor == 'lidar':
           
            calibration_file = self.root_split_path / 'lidar_calib' / ('%s.txt' % idx)
    ...
    '''
    self._camera_projection_matrix, self._T_camera_radar = self.get_sensor_transforms("这里输入idx",'radar')
    ...
    def get_radar_calib(self, idx):
        ...
        assert calib_file.exists()
        lidar_file = self.root_split_path / 'lidar_calib' / ('%s.txt' % idx)
        return calibration_kitti.Radar_Calibration(calib_file,lidar_file)
...
#    @staticmethod
    ...
    # # 利用t_radar_camera和t_camera_lidar参数,得到lidar转向radar坐标系下的值
    ...
    def get_infos(self, num_workers=4, has_label=True, count_inside_pts=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            # 从radar到lidar的转换矩阵
            info['t_lidar_radar'] = radar_calib.t_lidar_radar 
            ...
            if has_label:
                ...
                if count_inside_pts:
                    if self.pc_type == 'lidar' or self.pc_type == 'radar':
                        ...
                        # 返回true or false list判断点云是否在fov下，判断该点云能否有效 （是否用于训练）
                        radar_fov_flag = self.get_fov_flag(pts_rect_radar_points, info['image']['image_shape'], radar_calib)
                        # 提取有效点
                        radar_pts_fov = radar_points[radar_fov_flag ]
                        radar_points = radar_pts_fov
                        
                        compensated_radial_velocity = radar_points[:, 3:]
                        # compensated_radial_velocity = radar_points[:, 5]
                        # 这里还得将radar转为lidar点  
                        # 8.15 目前这里可能只有xyz坐标，没有速度，速度也得转换
                        radar_points_xyz = transform_pcl(points=radar_points,
                                                transform_matrix=radar_calib.t_lidar_radar)
                        radar_points_xyz = radar_points_xyz[:, 0:3]
                        radar_points = np.hstack((radar_points_xyz,
                        ...
                        if self.pc_type == 'lidar' or self.pc_type == 'radar':
                            ...
                            for k in range(num_objects):
                                lidar_flag = box_utils.in_hull(lidar_points[:, 0:3], corners_lidar[k])
                                lidar_num_points_in_gt[k] = lidar_flag.sum()
                                radar_flag = box_utils.in_hull(radar_points[:, 0:3], corners_lidar[k])
                        ...
                        if self.pc_type == 'lidar' or self.pc_type == 'radar':
                            ...
                            # 8.15
                            # 8.19 按道理这个不要
                            annotations['num_points_in_gt'] = lidar_num_points_in_gt
                    '''
                    if self.pc_type == 'lidar' or self.pc_type == 'radar':
                        num_points_in_gt = -np.ones(num_gt, dtype=np.int32)
                    else:
    ...
    def create_groundtruth_database(self, info_path=None, used_classes=None, split='train'):
        ...
        for k in range(len(infos)):
            ...
            if self.pc_type == 'lidar' or self.pc_type == 'radar':
                ...
                lidar_points, radar_points = self.get_pointcloud(sample_idx, self.pc_type)
                radar_calib = self.get_radar_calib(sample_idx)
                # 8.15：！！！现在radar只有xyz坐标 但是这个应该无所谓
                radar_points = transform_pcl(points=radar_points,
                                                transform_matrix=radar_calib.t_lidar_radar)
                ...
                for i in range(num_obj):
                    ...
                    if (used_classes is None) or names[i] in used_classes:
                        ...
                        db_info = {'name': names[i], 'lidar_path': lidar_db_path, 'radar_path': radar_db_path, 'image_idx': sample_idx, 'gt_idx': i,
                                'box3d_lidar': gt_boxes[i], 'lidar_num_points_in_gt': lidar_gt_points.shape[0],'num_points_in_gt': lidar_gt_points.shape[0], 'radar_num_points_in_gt': radar_gt_points.shape[0],
                                'difficulty': difficulty[i], 'bbox': bbox[i], 'score': annos['score'][i]}
                        if names[i] in all_db_infos:
                            all_db_infos[names[i]].append(db_info)
                        else:
    ...
    def __getitem__(self, index):
        ...
        if 'annos' in info:
            ...
            # 因为label中是相机camera坐标系
            # 8.15 这里应该也要换，因为有可能用的是radar的转换参数
        ...
        if "points" in get_item_list:
            if self.pc_type == 'lidar' or self.pc_type == 'radar':
                ...
                # velocity_vectors = get_radar_velocity_vectors(radar_points_xyz, compensated_radial_velocity)
                
                input_dict['radar_points'] = radar_points
                
                # input_dict['points'] = lidar_points
        ...
        # 8.15：这个地方fusion类别的话不知道如何处理
        # 8.15：因为真实
        if self.pc_type == 'lidar' or self.pc_type == 'fusion'  :
            if "calib_matricies" in get_item_list:
                input_dict["trans_lidar_to_cam"], input_dict["trans_cam_to_img"] = kitti_utils.calib_to_matricies(lidar_calib)
        if self.pc_type == 'radar':
            if "calib_matricies" in get_item_list:
                input_dict["trans_lidar_to_cam"], input_dict["trans_cam_to_img"] = kitti_utils.calib_to_matricies(radar_calib)
        
        # 8.15： 和上面对应，后面应该还是得该calib取消掉
        input_dict['calib'] = lidar_calib # 这个都是基于lidar坐标系来的，所以按lidar的来
